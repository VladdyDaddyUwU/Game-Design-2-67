function [Level, Player, Forces, Percentages, Deformation] = Function1(coords_player, elems_player)

%level-dependent values
level = 1;
Available_material = 1;
txt = "*arrow pointing to beam* In this first level, part of the structure has already been built for you. " + newline + ...
    "This beam will be part of a larger construction, made up of multiple beams connected by nodes on the grid points. Nodes are pin joints that hold the ends of the beams together." + newline + ...
    "*arrow pointing to first anchor node* The beam is connected to an anchor node. An anchor node is attached to the outside world and can not be displaced." + newline + ...
    "*arrow pointing to second anchor node* This node is the second anchor node. Anchor nodes can be recognised by the triangle symbol." + newline + ...
    "*arrow pointing to node with load* This node is the load-bearing node. A weight is attached to this node, and your goal is to prevent this weight from falling." + newline + ...
    "*arrow pointing to inventory* To complete the structure, you are only given one meter of steel beam as shown here in your toolbox. You can implement a new beam using your available material by drawing a line between two nodes on the grid." + ...
    "Well, get to work! Let's see what you've got.";
